...
class DeceptionNet(nn.Module):
    def __init__(self, input_dim=INPUT_DIM, global_dim=GLOBAL_DIM, hidden_dim=HIDDEN_DIM):
        ...
        # Simplified bidirectional GRU with 7 layers
        self.gru = nn.GRU(128, hidden_dim, num_layers=9, batch_first=True,  # Increased to 7 layers
    ...
    def forward(self, x, g, lengths):
        ...
        # Apply layer normalization (gru_out is already unpacked if we're on CUDA)
...
def train_for_folder(folder_name):
    ...
    log_dir = os.path.join("logs_GRU", folder_name)
    ...
    try:
        ...
        try:
            ...
            log_dir_score = os.path.join("logs_GRU", new_folder_name)
        ...
        if "CUDA out of memory" in str(exc):
            ...
            raise exc
        elif "CUDNN_STATUS_NOT_SUPPORTED" in str(exc):
...
if __name__ == '__main__':
    for folder in ["data_new_truncated_final", "data_new_truncated", "data_new"]:  # "data_combined",
        # "data_reconstructed"]:
        try:

...
BATCH_SIZE = 4  # Moderate batch size for better gradient estimates
EPOCHS = 100
NUM_WORKERS = 2
PATIENCE = 15
DROPOUT = 0.5
...
def train_for_folder(folder_name, use_smote=True):
    ...
    log_dir = os.path.join("logs_GRU_optimized_SMOTE", folder_name)
    ...
    print(f"[yellow]Starting {n_folds}-fold cross-validation")
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels)):
        ...
        train_labels_final = [labels[i] for i in train_idx_final]
        val_labels = [labels[i] for i in val_idx]
        test_labels = [labels[i] for i in test_idx]
        
        print(f"Train set: {len(train_set)} samples, distribution: {Counter(train_labels_final)}")
        ...
        if use_smote and SMOTE_AVAILABLE:
            ...
            # Option 1: Recalculate balanced weights (if SMOTE didn't create perfect balance)
            class_weights = compute_class_weight('balanced', classes=np.unique(train_labels), y=train_labels)
            class_weights = torch.tensor(class_weights, dtype=torch.float32)
            print(f"[green]Updated class weights after SMOTE: {class_weights}")

            # Option 2 (alternative): Use uniform weights if classes are perfectly balanced
            # class_weights = torch.ones(len(np.unique(train_labels)), dtype=torch.float32)
        ...
        try:
            ...
            delta = 0.001
            best_model_path = os.path.join(log_dir, f"best_model_fold{fold+1}.pt")
            ...
            for epoch in range(1, EPOCHS + 1):
                ...
                # Validate
                v_loss, v_acc, _, _, v_f, _, _, _ = evaluate(model, val_loader, criterion)
            ...
            plt.title(f'Fold {fold+1} - F1 Score History')
            plt.xlabel('Epoch')
            plt.ylabel('F1 Score')
            plt.legend()
            
            plt.tight_layout()
            plt.savefig(os.path.join(log_dir, f"fold{fold+1}_history.png"))
            plt.close()
            
            # Evaluate on test set
            print("\nLoading best model for test evaluation...")
            model.load_state_dict(torch.load(best_model_path))
            _, test_acc, test_p, test_r, test_f, y_true, y_pred, y_probs = evaluate(model, test_loader, criterion)
            ...
            print(f"\nFold {fold+1} Test: Acc={test_acc:.4f} "
            ...
            plt.ylabel('True')
            plt.tight_layout()
            plt.savefig(os.path.join(log_dir, f"fold{fold+1}_confusion_matrix.png"))
            plt.close()
    ...
    if fold_results:
        ...
        print("\n[green]Cross-validation results:")
        ...
        # Combine probabilities across folds
        all_probs = np.concatenate([fold['y_probs'][:, 1] for fold in fold_results])
        all_true_roc = np.concatenate([fold['y_true'] for fold in fold_results])
        
        fpr, tpr, _ = roc_curve(all_true_roc, all_probs)
        auc = roc_auc_score(all_true_roc, all_probs)
        ...
        # Select the best fold based on F1 score
        best_fold_idx = np.argmax([fold['test_f1'] for fold in fold_results])
        best_fold = fold_results[best_fold_idx]
        print(f"[green]Using best model from fold {best_fold['fold']} with F1: {best_fold['test_f1']:.4f}")

        # Copy the best model to be the final model
        best_model_path = os.path.join(log_dir, f"best_model_fold{best_fold['fold']}.pt")
        final_model_path = os.path.join(log_dir, "final_model.pt")
        import shutil
        shutil.copy(best_model_path, final_model_path)

        # Also create a CPU version
        model = DeceptionNet().to('cpu')
        model.load_state_dict(torch.load(best_model_path, map_location='cpu'))
        torch.save(model.state_dict(), os.path.join(log_dir, "final_model_cpu.pt"))

        # Rename folder with test accuracy from the best fold (to match original structure)
        if os.path.exists(log_dir):
            best_acc = best_fold['test_acc']  # Use accuracy from best fold instead of average
            new_folder_name = f"{folder_name} ({best_acc*100:.2f}% Test Acc)"
            new_log_dir = os.path.join("logs_GRU_optimized_SMOTE", new_folder_name)
            try:
